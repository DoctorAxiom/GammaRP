...
\GRPbitepar{0mm,1mm,12mm,13mm,13mm,8mm}\GRPparabgsvg[height=2cm,shift={(8mm,0)},side=left]{art/twodice}
...
\GRPbitepar[right]{0pt,8.9mm,10.5mm,9.5mm}\GRPparabgsvg[height=1.4cm,side=right,shift={(-1.6cm,0pt)}]{art/onedice}
...
\GRPparabgsvg[height=1.6cm,side=right,shift={(-1.1cm,-4mm)}]{art/onedice_b}
...
When this happens, it is often easier as a GM to specify the roll they must make as a modification of their usual roll.

There are two ways to modify a roll- you can modify the number of dice a character has, or you can modify the advantage they have on the roll.

\subsection{Bonus and Penalty Dice}
In general, you should only modify the number of dice a character has for a roll if the rulebook specifies that you should do so. Modifying the number of dice is described using the terms ``die bonus'' and ``die penalty''.

\index{Die Penalty}If the rules specify you take a \emph{W} die penalty- then you roll with \emph{W} fewer dice than you ordinarily would.

\index{Die Bonus}If the rules specify you take a \emph{X} die bonus- then you roll with \emph{X} more dice than you ordinarily would.

\subsection{Additional Advantage and Disadvantage}
Modifying the advantage and disadvantage on a roll may be done more freely by the GM. If the situation is harder than usual, the GM may impose ``additional disadvantage'' on the players. If the situation has been modified to be easier than usual, the GM may grant ``additional advantage'' to the players.

\index{Additional Advantage}If the GM or the rules specify you take \emph{Y} additional advantage- then you roll with \emph{Y} higher advantage than you ordinarily would.

\index{Additional Disadvantage}If the GM or the rules specify you take \emph{Z} additional disadvantage- then you roll with \emph{Z} lower advantage than you ordinarily would. Remember that in the roll notation \emph{-Z} advantage is the same as \emph{Z} disadvantage. Rolling with additional disadvantage will not often make the absolute roll a disadvantaged roll, there will still be some net advantage to the roll left over after applying some minor additional disadvantage.
...
\GRPsay{GM}{As you enter the swamp, the mud grows deeper and harder to trudge through. You'll all have to use your athletics talent to pass without hurting yourselves.}
\GRPsay{Alice}{Can we make this a group roll? I think we could help each other cross.}
\GRPsay{GM}{Absolutely. Make a group roll for athletics.}
\GRPrheader{Athletics Group Roll}
\GRPsay{Bob}{\GRProll{3}{3} - \GRPdicelist[3]{8,8,3}}
\GRPsay{Alice}{\GRProll{5}{5} - \GRPdicelist[5]{3,8,2,3,10}}
\GRPsay{Eve}{\GRProll{6}{4} - \GRPdicelist[2]{3,8,12,3,1,2}}
\GRPrfooter{}
...
\index{Roll Contest}Sometimes, one character may be asked to dynamically struggle against the abilities of another character or entity within the game. When this happens, the game enters a roll contest-- where both characters roll against each other and apply penalties to the other's roll depending on how well they do.
...
\GRPsay{GM}{Sure! Go ahead and initiate a Roll Contest with Dexterity.}
\GRPsay{Eve}{I'll roll with Athletics since I'm better at that.}
\GRPsay{GM}{Sure, you can \emph{talent proxy}. Use Athletics at 2 additional disadvantage.}
