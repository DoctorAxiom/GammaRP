...
\GRPbitepar{0mm,1mm,12mm,13mm,13mm,8mm}\GRPparabgsvg[height=2cm,shift={(8mm,0)},side=left]{art/TheBasics/twodice}
...
\begin{itemize}
	\item \textit{X} is the number of dice you roll.
	\item \textit{Y} is the \emph{advantage}\index{Advantage} on the roll, which indicates that the top \textit{Y} numbers of a die pass the success threshold.
\end{itemize}
...
An advantage of 0 is the exception to this: \GRProll{X}{0} is still successful if any of the dice roll a 12.
Both larger \textit{X} and larger \textit{Y} indicate a higher probability of success.
...
When the disadvantage is greater than or equal to the number of dice being rolled, the roll is an automatic failure even if you have at least one dice showing a 12. If you are asked to make a roll with a disadvantage equal to or greater than the number of dice, you may simply declare a failure. However you may still make the roll if you have abilities that benefit from actually rolling the dice.
...
\index{Group Roll}Sometimes multiple characters must all individually perform the same task, in a way where their average performance impacts the success of the group. This often happens in situations where one character could catch another character if they fail, by performing well enough.
Such a situation calls for a Group Roll.
...
\GRPsay{Alice}{Can we make this a Group Roll? I think we could catch each other if we fall.}
\GRPsay{GM}{Absolutely. Make a Group Roll for \GRPtalent{Fortitude}.}
...
Other examples of situations where group rolls are appropriate include:
\begin{itemize}
	\item Stealthily getting through a stretch of terrain
	\item Persuading someone that your group is trustworthy
	\item A coordinated group performance of any kind
	\item Crafting a large batch of items or cooking a large meal
\end{itemize}
...
Step \ref{contest:last} is only important when one of the characters has an ability that imposes a penalty for failing a roll.
...
\GRPsay{GM}{Sure! Go ahead and initiate a Roll Contest with \GRPtalent{Athletics}.}
...
\section{Helping Rolls}

\index{Helping Roll}Sometimes in a situation, a character may be able to help another character perform a task, such as lifting a heavy object, searching for insight into a puzzle, playing a game of chess, putting on a performance, or wrestling an opponent.

When this happens they may roll to help the other character. This is simpler than a Group Roll.

An helping roll is performed as follows:

\begin{enumerate}
	\item Any character that is able to help and wants to help rolls the same talent or ability as the character making the initial roll.
	\item The sum of all participating characters' successes, including the original character, is the number of successes for the original roll.
	\item If the roll risks injury on failure, all of the participating characters will receive the injury.
\end{enumerate}

Below is an example of a helping roll.

\begin{GRPquicknote}
\index{Helping Roll!Example}
\GRPsay{GM}{Large Larry is the only one left, and he is \emph{obstructing} the exit to this room. Player party, it's your offensive phase.}
\GRPsay{Eve}{I'm so done with this fight. Can I just cast telekinesis and shove him out of the way?}
\GRPsay{Bob}{I also have telekenesis. I can help!}
\GRPsay{GM}{Great, this will be a roll contest between Eve and Large Larry, with Bob helping Eve. Both of you give me your telekinesis rolls and Larry will contest with fortitude.}
\GRPrheader{Roll Contest Round 1}
\GRPsay{Eve}{\GRProll{4}{4} - \GRPdicelist[4]{9,3,4,9} }
\GRPsay{Bob}{\GRProll{4}{3} - \GRPdicelist[3]{5,11,10,10}}
\GRPsay{Large \\ Larry}{\GRProll{6}{5} - \GRPdicelist[5]{7,2,1,12,9,1}}
\GRPrfooter{}
\GRPsay{GM}{Nice rolls. Larry will take \GRPdicepenalty{5} next round, Bob and Eve will both take \GRPdicepenalty{3} next round. }
\GRPrheader{Roll Contest Round 2}
\GRPsay{Eve}{\GRProll{1}{4} - \GRPdicelist[4]{9} }
\GRPsay{Bob}{\GRProll{1}{3} - \GRPdicelist[3]{4}}
\GRPsay{Large \\ Larry}{\GRProll{1}{5} - \GRPdicelist[5]{4}}
\GRPrfooter{}
\GRPsay{GM}{Excellent work. You push Larry out of the way and he is no longer obstructing the exit.}
\end{GRPquicknote}

Note that this was a helping roll used in a roll contest. In round 2, both Bob and Eve took the penalty from Larry's successes in round 1, but Larry took the penalties from both Bob and Eve's successes in round 1.

When helping rolls are used for abilities other than base talents, the helping character must in general have and actively use the same ability as the character they are helping.

For instance, to help another character cast a particular spell, the helping character must know the same spell and use the same cast and consume a cast use.

In order to help another character use a specific augment ability, the helping character must have the same augment active in their loadout and use the same ability.

Additionally, to help a character in phased mode who is using an ability that takes an action (offensive or defensive), the helping character must also consume an action to help.
